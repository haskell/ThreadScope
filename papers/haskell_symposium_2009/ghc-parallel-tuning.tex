...
\cite{dph}.  Runtime performance has received concerted attention
recently, with significant improvements in parallel performance coming
in forthcoming GHC releases \cite{multicore-ghc}.  Many of the runtime
bottlenecks that hampered parallel performance are much reduced, with
the result that it should be easier to achieve parallel speedups.
...
performance; for example the granularity should be not too fine nor
too coarse.  The runtime does everything it can to broaden the window
of granularities that yield good performance, but it cannot completely
elimiante the upper and lower bounds.
...
and tools to communicate this information back to the programmer in a
