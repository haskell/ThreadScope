 ...
 * EventType :
 *       EVENT_ET_BEGIN
 *       EventTypeNum      -- unique identifier for this event
 *       Word32 (= len)    
 *       Word8[len]        -- string describing the event
 *       Word16            -- size of the event in bytes (minus the header)
 *       EVENT_ET_END
 *
 * Event : (see struct definitions below)
 *
 *
 * To add a new event
 * ------------------
 ...
 *    - emit the new event in psotEvent_()
...
#define EVENT_DATA_END        0xffffffff
...
typedef StgWord32 EventTypeNum;
